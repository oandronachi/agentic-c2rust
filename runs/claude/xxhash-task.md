...
```yaml
lib_name:        xxhash                                # -> crates xxhash-rs / xxhash-sys / xxhash-cabi
output_dir:      claude                                # where to create the ${LIB}-rs-migration/ workspace:
                                                       #   "." (default) = same folder as this playbook
                                                       #   or an absolute/relative path the user selects
upstream_url:    https://github.com/Cyan4973/xxHash    # vendored file: xxhash.h (single-header)
upstream_pin:    v0.8.3                                # real release tag -> reproducible (unlike a branch)
license:         BSD-2-Clause                          # library files xxhash.c/.h are BSD-2-Clause
                                                       # (NOTE: the xxhsum CLI is GPL — do NOT vendor it)
headers:         ["xxhash.h"]                          # compile the impl via #define XXH_IMPLEMENTATION
api_functions:   ["XXH32", "XXH64"]                     # one-shot, fully stable & specified hashes
                                                       # (XXH3_64bits/XXH128 also stable since v0.8.0 if wanted)
opaque_types:    []                                    # none for one-shot; XXH3_state_t only for streaming
allocator:       caller-provided                       # returns a u32/u64 by value — nothing is heap-allocated
determinism:     deterministic_bytes                   # hash is a pure function of (input bytes, seed)
oracle_relation: byte_exact                            # equivalence = identical hash value for same input+seed
```
