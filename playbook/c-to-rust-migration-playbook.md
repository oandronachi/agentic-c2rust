...
```mermaid
flowchart TD
    P0["Phase 0<br/>intake + env probe"] --> P1["Phase 1<br/>characterize C lib<br/>choose oracle relation"]
    P1 --> P2["Phase 2<br/>scaffold 4-crate workspace"]
    ...
    P9 --> P10["Phase 10<br/>cleanup ledger"]

    classDef setup fill:#dfe7fd,stroke:#1f3a93,color:#000
    classDef build fill:#fff3cd,stroke:#856404,color:#000
    classDef verify fill:#e8f5e9,stroke:#1b5e20,color:#000
...
Setup phases (blue) lock parameters. Build phases (amber) cross the FFI boundary in each direction. Verify phases (green) prove equivalence and measure cost. Ship phases (pink) hand the result back to a reader and clean up after the agent.
...
```yaml
lib_name:        ${LIB}              # short slug -> crates ${LIB}-rs / ${LIB}-sys / ${LIB}-cabi
output_dir:      ${OUTPUT_DIR}       # where to create the ${LIB}-rs-migration/ workspace:
                                     #   "." (default) = same folder as this playbook
                                     #   or an absolute/relative path the user selects
upstream_url:    ${UPSTREAM_URL}     # git/source URL of the C library
upstream_pin:    ${COMMIT_OR_TAG}    # exact commit/tag to vendor (reproducibility)
license:         ${SPDX}             # upstream license; confirm vendoring is allowed
headers:         [${HEADER_FILES}]   # public headers to bind, e.g. ["lib.h"]
api_functions:   [${FUNCS}]          # functions to port + bind, e.g. ["lib_process","lib_reset"]
opaque_types:    [${TYPES}]          # structs/handles crossing the boundary
